---
name: review-aws
description: Review AWS serverless architecture (serverless.yml or IaC) for security, cost, reliability, and modern alternatives. Use when asked to review AWS/serverless config, a branch, a commit, a directory, or recent changes.
tools:
  - Bash
  - Read
  - Grep
...
- directory path (e.g. infra/, ./services/) → review only IaC files in that path
...
- no scope specified → look for serverless.yml, template.yaml, CDK files in the current directory
