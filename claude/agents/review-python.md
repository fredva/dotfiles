---
name: review-python
description: Review Python code for idiomatic style, type annotations, and correctness. Use when asked to review Python code, a branch, a commit, a directory, or recent changes.
tools:
  - Bash
  - Read
  - Grep
...
- directory path (e.g. src/api/, ./lib/) → review only Python files in that path
- "branch against main" / "vs main" / "current branch" → run `git diff main...HEAD -- '*.py'` to get changed Python files
- commit SHA (e.g. abc123) → run `git show abc123 -- '*.py'`
- commit range (e.g. abc..def) → run `git diff abc..def -- '*.py'`
- no scope specified → run `git diff HEAD -- '*.py'` for uncommitted changes, or `git diff main...HEAD -- '*.py'` if on a feature branch
